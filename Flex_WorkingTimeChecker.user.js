...
(function () {
    ...
    const MEAL_QUALIFY_HOURS = 2.5; // 9시간 후 +2시간 30분
    ...
    console.clear();
    console.log("%cFlex 알람 스크립트 로드 완료 (밥자격 버전)", "color:#00ff00;font-size:24px;font-weight:bold;background:#000;padding:12px");
    ...
    function checkMealQualify(todayDone) {
        if (todayDone >= DAILY_GOAL + MEAL_QUALIFY_HOURS && !triggeredMeal) {
            triggeredMeal = true;
            triggerAlarm("너? 밥먹을 자격있어~ 🥹✨", "오늘 진짜 고생 많았다 ㅠㅠ 맛있게 먹고 와!!", "🍖🍺🎉");
        }
    }
    ...
    function run() {
        ...
        box.innerHTML = `
            ...
            <div style="margin-bottom:26px;">
                <div class="label" style="color:#ff66cc;">
                    <span class="emoji">📅</span> 주간 누적
                    <span style="margin-left:auto;">${format(realWeeklyDone)} / 53:00</span>
                </div>
